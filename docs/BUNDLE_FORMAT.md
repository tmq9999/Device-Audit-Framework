...
- `schema_version`: `"1.0"` for Phase 1, `"2.0"` for Phase 2, `"3.0"` for
  Phase 3 metadata, `"4.0"` for Phase 4 metadata, or `"5.0"` for Phase 5
  metadata.
...
- `collector_states` in schemas `2.0`, `3.0`, `4.0`, and `5.0`. Schema `3.0`
  adds `camera`, `sensors`, and `hal`; schema `4.0` adds `audio`, `battery`,
  `thermal`, and `storage`; schema `5.0` adds `network`, `graphics`, `input`,
  and `memory`.
...
The loader accepts schemas `1.0`, `2.0`, `3.0`, `4.0`, and `5.0`. Schema `1.0`
bundles have no `collector_states`; offline analysis derives legacy section
states from command evidence and treats absent optional sections as
`not_evaluated`. Schema `2.0` adds Phase 2 collector-state metadata. Schema
`3.0` adds Phase 3 collector states and accepts the `unsupported` state used
for Android-version dependent camera, audio-policy, battery, thermal, and
storage commands. Schemas `4.0` and `5.0` are additive; older command and
artifact representations remain unchanged and are never rewritten.
...
`LinkAddresses` lists, and `inet`/`inet6` address lines before persistence.
